 ...
 *
 * Conditions render as icons (sun, moon, cloud, fog, rain, snow, lightning) keyed
 * off the WMO weather code via `glyphFor`, with `labelFor` giving the short
 * english word.
 */
Singleton {
    id: root

    readonly property string cacheDir: (Quickshell.env("XDG_CACHE_HOME") || (Quickshell.env("HOME") + "/.cache")) + "/lumen"
    ...
    /**
     * Maps a WMO weather code to its icon name. Clear skies show a moon at night
     * so the glance reads day-versus-night at a glance; every other condition is
     * the same icon round the clock.
     */
