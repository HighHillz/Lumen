 ...
 * `$XDG_CACHE_HOME/lumen/rec-thumbs`, skipping clips already cached) and only
...
Singleton {
    ...
    readonly property string thumbDir: (Quickshell.env("XDG_CACHE_HOME") || (home + "/.cache")) + "/lumen/rec-thumbs/"
    ...
    Process {
        ...
        onExited: function(exitCode) {
            ...
            if (exitCode !== 0) {
                ...
                failProc.command = ["notify-send", "-a", "Lumen", "-u", "critical",
    ...
    Process {
        id: savedProc
        command: ["notify-send", "-a", "Lumen", "Recording saved",
