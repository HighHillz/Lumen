...
flags_file="${XDG_STATE_HOME:-$HOME/.local/state}/lumen/flags.json"
...
map_put() {
  mkdir -p "$(dirname "$MAP")"
  {
    awk -F'\t' -v o="$1" '$1 != o' "$MAP" 2>/dev/null || true
    printf '%s\t%s\n' "$1" "$2"
  } >"$MAP.tmp"
  mv "$MAP.tmp" "$MAP"
}
...
sync_videos() {
  ...
  while IFS=$'\t' read -r o pic; do
    [ -n "$o" ] || continue
    setsid -f mpvpaper -p -o "no-audio loop-file=inf hwdec=auto panscan=1.0 --load-scripts=no" "$o" "$pic" >/dev/null 2>&1
  done <<<"$desired"
}

# The palette follows the focused monitor: whatever hangs there drives the
# global palette state and global still, so the Settings dynamic re-run
...
palette_update() {
  ...
  # Kitty appends the process PID to the listen_on socket, so discover
  # each socket dynamically instead of hardcoding a PID.
  for kitty_pid in $(pgrep -x kitty 2>/dev/null || true); do
    kitten @ --to "unix:@mykitty-$kitty_pid" load-config >/dev/null 2>&1 || true
  done

  # Repaint all running interactive Fish shells so Starship picks up
  # the newly generated ~/.config/starship.toml immediately.
  pkill -WINCH -u "$USER" fish 2>/dev/null || true
}
